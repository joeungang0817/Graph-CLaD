...
MARGIN = 0.02
# Calibrated for the LIBERO Panda gripper in the Colab robosuite runtime:
# open baseline is about |qpos|=0.04, while a grasped object keeps one or both
# fingers around |qpos|=0.015--0.025 instead of reaching the zero target.
...
def _geometry_record(
    ...
    node_types = {
        str(entry.get("logical_id")): str(entry.get("node_type"))
        for entry in object_states
        if isinstance(entry, Mapping)
    }
    if relation == "inside" and not bool(target_box.get("containment_capable", False)):
        return relation_record(None, "geometry_inside_requires_explicit_container_capability")
    if relation == "on" and not bool(target_box.get("support_surface_candidate", False)):
        return relation_record(None, "geometry_on_requires_support_surface_candidate")
