...
try:
    from ..phase1.state_inspection import (
        _infer_action_dim,
        _jsonable,
        _sim_metadata,
                ...
                if body_id not in body_names:
                    try:
                        ...
                        if isinstance(raw_body_name, bytes):
                            raw_body_name = raw_body_name.decode("utf-8", errors="replace")
                        body_name = str(raw_body_name)
                    except Exception:
                        body_name = ""
                    # Different robosuite/MuJoCo builds expose finger bodies
                    # with names such as ``gripper0_leftfinger`` or
                    # ``gripper0_finger_joint1_tip``.  Treat either token as
                    # the stable robot node, even if a wrapper adds a prefix
                    # or returns a byte string.
                    if "robot0" in body_name or "gripper0" in body_name:
