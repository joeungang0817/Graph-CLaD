...
try:
    from ..phase2a.graph_extractor import _make_edges, extract_graph_snapshot
...
def _select_nodes(graph: Mapping[str, Any], include_all_sites: bool) -> list[dict[str, Any]]:
    ...
    for original in graph["nodes"]:
        ...
        if not include_all_sites and node["node_type"] == "site":
            continue
        # Keep the compatibility slot at index 4 zero, but remove the
        # task-derived metadata key from the model-input feature dictionary.
        # The relevance signal is audit/ablation metadata only, never input.
        node["features"].pop("is_object_of_interest", None)
                ...
                if relation in GEOMETRIC_RELATIONS
                    ...
                    if relation == "contact"
                        ...
                        if relation in {"on", "inside"}
                            ...
                            f"gripper_contact_and_abs_qpos_le_{gripper_closed_threshold:g}"
                            if relation == "holding"
...
def _parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=float,
        default=GRIPPER_CLOSED_THRESHOLD,
        help="Absolute gripper qpos threshold used with robot-object contact.",
    )
