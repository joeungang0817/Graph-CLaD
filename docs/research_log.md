...
- The first torch-enabled regression run after the conditional edge-module fix
  found a second dead-parameter path before any v5 training: the common
  structured base still instantiated `node_pool` for every encoder, while
  `PairPastAct` used its separate `pair_pool` (and non-message `RelPoolPastAct`
  used `edge_pool`). The gradient-coverage test therefore failed on
  `C3-Pair-PastAct` at `node_pool.score.{weight,bias}` and would have exposed
  the analogous RelPool pool next. Pool construction is now owned by the
  concrete architecture: SceneSet and MPNNs create `node_pool`, Pair creates
  only `pair_pool`, and RelPool creates only `edge_pool`. The active forward
  computations are unchanged; only unreachable parameters are removed. All
  width/parameter-match reports must consequently be regenerated by the v5
  smoke, and no v5/full run may start until the complete SSH torch suite passes.

## 2026-08-17 — Phase 3C post-integrity implementation (local, SSH pending)

- The running corrected `core_smoke_v5` process is deliberately left
  untouched. Its result may be retained as dead-parameter/capacity smoke
  evidence, but it predates the stronger semantic-store and checkpoint
  provenance contract below and cannot be promoted into a formal result.
- Semantic feature stores are now immutable once any output exists. Every
  declared shard is checked against the manifest SHA-256 map before Base/Core
  training. The shard inventory must match exactly; missing, extra, or modified
  `.npz` files fail startup.
- Camera acceptance is now a machine-enforced gate rather than a log-only
  statement. The human attestation must accept the configured external and
  wrist views and hash both the passing determinism QA JSON and the exact
  reviewed contact sheet. The already extracted semantic store does not need
  regeneration, but the SSH attestation must be re-emitted once so it contains
  the new contact-sheet hash.
- Base checkpoint, resume checkpoint, and runtime schemas advance to v4 and
  carry a canonical semantic-store-integrity/attestation hash. Core schema v4
  requires an exact match between its current store verification and the Base
  checkpoint. Thus the previous Base smoke checkpoint cannot be accidentally
  used by a formal Core run. New artifact roots are
  `base_clad_smoke_v5`, `base_clad_v5`, `core_smoke_v6`, and
  `core_screen_v6`.
- Completed-run reuse now verifies current runner source, trainer source,
  transitive Phase3C code identity, and hashes for best/resume checkpoints,
  metrics, predictions, stdout, and stderr. Formal configs require a clean git
  checkout. Runtime manifests record start/end git provenance, Python/package
  versions, device/determinism/AMP state, memory, and throughput.
- The structured adapter is now handed a newly constructed causal object that
  contains only `graph_prev`, `graph_current`, and `past_action`; future
  targets and task metadata are not merely ignored but absent. A poison test
  changes every future target and requires bit-identical outputs while an
  inspection encoder confirms that target and task attributes are unavailable.
- Structured-model verification now covers full-scene set invariance, Pair
  invariance under non-robot entity reordering, Geom/Rel MPNN invariance,
  RelPool–RelMPNN raw edge-token parity, adapter-off independence, absence of
  dead trainable parameters, and a tiny fixed-batch overfit test. These new
  tests require the SSH torch environment.
- Evaluation prediction rows now include `prev_step`, `current_step`,
  `target_step`, `tau`, fold/seed identity, fold-train prevalence, and the
  frozen validation threshold. The analyzer combines multiple fold artifacts
  without requiring identical thresholds, enforces configured fold/seed
  completeness, verifies model identity, hashes all inputs, and reports
  inverse-relation family metrics, no-change and train-prevalence baselines,
  primary RelMPNN contrasts, paired task/episode bootstrap, and a
  `prev_step=0` exclusion sensitivity.
- A versioned uniform action-timing config was added for early/middle/late
  probes at the frozen tolerance 1.05. The known task0 test initial-transition
  outlier is preserved; the code separately reports initial and non-initial
  failures instead of silently relaxing tolerance.
- The joined-manifest example config now targets the already attested builder
  candidate path, not the legacy fixed semantic-store source. Both joined and
  semantic builders reject overwriting existing/partial outputs. A full
  six-model, three-fold seed-0 analysis config and replay-only dependency file
  were added.
- Local verification after this implementation compiled all Phase3C files,
  parsed all seven affected JSON configs, and ran 63 tests: 35 passed and 28
  torch-dependent tests were skipped because the Windows runtime has no
  PyTorch. No local test failed. The mandatory next gate is the full 63-test
  suite on SSH, followed by new Base v5-path and Core v6-path smoke runs.
- A model-agnostic Phase 4 residual foresight interface was also implemented
  without prematurely choosing a Phase 3C winner. It splits the semantic CLaD
  foresight into proprio/scene branches, adds separate zero-initialized causal
  structured residuals, normalizes each branch, and returns the same `[B,2H]`
  Stage-2-facing tensor for semantic and graph variants. Adapter-off skips the
  structured encoder entirely; zero alpha must be exactly equal to the
  semantic interface; nonzero alpha must respond to causal structured input.
  Stage 2 remains intentionally unimplemented because the preregistered design
  forbids it before this Phase 4 equivalence passes in the torch environment
  and before the Phase 3C winner/fairness control are selected.
- The three Phase 4 contract tests compile and are collected locally; all three
  are skipped only because the Windows runtime has no PyTorch. Together with
  the scoped Phase 3C run, the final local gate is 35 passed, 31 torch-skipped,
  and zero failures across 66 collected Phase3C/Phase4 tests.

## 2026-08-17 — Deadline waiver: retain corrected Core v5 as final smoke

- With submission time constrained, the user chose not to repeat the complete
  six-model smoke solely to exercise the post-integrity artifact schema. The
  corrected `core_smoke_v5` is accepted as the final technical wiring smoke if
  all six candidates completed with finite losses, no dead-gradient failure,
  passing parameter matching, and intact checkpoint/metric/prediction files.
- This waiver is justified narrowly: the subsequent changes physically narrow
  the adapter input to fields the existing encoders already read and strengthen
  store/checkpoint/provenance/analysis gates; they do not change the default
  enabled adapter equations. Therefore a numerically similar smoke result is
  expected. The new 66-test SSH suite substitutes for a redundant v6 smoke.
- The waiver does not grandfather v5 smoke artifacts into performance
  evidence. Their scores remain ineligible for selection, their checkpoints
